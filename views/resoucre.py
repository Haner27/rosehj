...
def article_edit():
    ...
    if mode == 'new':
        ...
        if emails:
            html = """
            Hi, I wrote a new article in in <b>HJ Rose Fashion Critic</b>, come here <a href="http://rosehj.com/d/{0}">>><b>click me</b></a>
            -- <b>RoseHJ</b>
            """.format(c.id)
            from utils.mail_utils import Email
            e = Email('ink_tech@126.com', [str(e) for e in emails], sender='HJ Rose Fashion Critic',
...
def comment_reply():
    ...
    if parent_id:  # 子回复
        ...
        notify_follow_up_s = r.notify_follow_up if r else None
        to_nickname = r.nickname if r else '-'
        to_email = r.email if r else '-'
    else:  # 一级回复
        ct = CommentText.objects(id=comment_id).first()
        notify_follow_up_s = ct.notify_follow_up if ct else None
    ...
    if notify_follow_up_s:
        html = """
        Hi, you have a new reply in <b>HJ Rose Fashion Critic</b>, check it <a href="http://rosehj.com/d/{0}">>><b>click me</b></a>
         -- <b>RoseHJ</b>
        """.format(content_id)
        from utils.mail_utils import Email
        e = Email('ink_tech@126.com', [to_email], sender='HJ Rose Fashion Critic',
                  subject='Have a new reply!', body=None, html=html)
        e.send_email()
